...
namespace ChatWithDocs.Infrastructure.Embeddings;

public class VoyageEmbeddingClient(HttpClient httpClient) : IEmbeddingService
{
    ...
    private async Task<List<float[]>> EmbedAsync(IReadOnlyList<string> texts, string inputType, CancellationToken cancellationToken)
    {
        ...
        foreach (var batch in texts.Chunk(MaxBatchSize))
        {
            var request = new VoyageEmbeddingRequest
            {
                ...
                Model = Model,
                InputType = inputType,
                OutputDimension = Constants.EmbeddingDimensions,
            };
